...
print("COUNT NULL IN ROW")

print("# Count null entries in each column")
null_counts = df1.select([sum(col(c).isNull().cast("int")).alias(c) for c in df1.columns])

null_counts.show()

print("# Remove the row with null entires and store them in a new dataframe named df2")
df2 = df1.filter(col("age").isNull())
df2.show()


print("#create a new dataframe df3")
...
print(""" #fill the null value with the mean age of students
...
print("Get the students who are 18 years or older")
